...
import { TrainingSeries, CreateTrainingSeriesDto } from "@/types";
import styles from "./styles.module.scss";

interface EditSeriesModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: Partial<CreateTrainingSeriesDto>) => void;
...
const EditSeriesModal: React.FC<EditSeriesModalProps> = ({
  ...
  const [formData, setFormData] = useState<Partial<CreateTrainingSeriesDto>>({
    title: "",
    description: "",
    difficulty: "beginner",
    type: "recurring",
  ...
  useEffect(() => {
    if (series) {
      setFormData({
        ...
        repeatTime: series.repeatTime || "19:00",
        duration: series.duration || 120,
        startDate: series.startDate || "",
        endDate: series.endDate || "",
        defaultLocation: series.defaultLocation || "",
        defaultMinParticipants: series.defaultMinParticipants,
        defaultMaxParticipants: series.defaultMaxParticipants,
        trainingProgramId: series.trainingProgramId,
      });
    }
  }, [series]);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="시리즈 수정">
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <div className={styles.inputGroup}>
          <label htmlFor="trainingProgramId">연결할 훈련 프로그램 *</label>
          <select
